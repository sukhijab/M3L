...
class VTMAE(nn.Module):
    def __init__(
        ...
        sample_image = torch.zeros((1, self.encoder.image_height // self.encoder.image_patch_height,
                                    self.encoder.image_width // self.encoder.image_patch_width, encoder_dim))
        ...
        sample_tactile = torch.zeros((1, self.encoder.tactile_height // self.encoder.tactile_patch_height,
                                      self.encoder.tactile_width // self.encoder.tactile_patch_width, encoder_dim))

        tactile_pos_embedding = enc_pos_embedding(sample_tactile).flatten(1, 2)  # 1 x 1tactile_patches x encoder_dim

        self.register_buffer('tactile_enc_pos_embedding', repeat(tactile_pos_embedding, 'b n d -> b (v n) d',
                                                                 v=self.num_tactiles))  # 1 x tactile_patches x encoder_dim

        sample_image = torch.zeros((1, self.encoder.image_height // self.encoder.image_patch_height,
                                    self.encoder.image_width // self.encoder.image_patch_width, decoder_dim))
        ...
        sample_tactile = torch.zeros((1, self.encoder.tactile_height // self.encoder.tactile_patch_height,
                                      self.encoder.tactile_width // self.encoder.tactile_patch_width, decoder_dim))
        tactile_pos_embedding = enc_pos_embedding(sample_tactile).flatten(1, 2)  # 1 x 1tactile_patches x decoder_dim
        self.register_buffer('tactile_dec_pos_embedding', repeat(tactile_pos_embedding, 'b n d -> b (v n) d',
                                                                 v=self.num_tactiles))  # 1 x tactile_patches x decoder_dim
    ...
    def forward(self, x, use_vision=True, use_tactile=True):
        ...
        if self.num_tactiles > 0 and use_tactile:
            ...
            for i in range(self.num_tactiles):
                if self.use_sincosmod_encodings:
                    tactile_tokens[:, i * num_single_tactile_patches:(
                                                                                 i + 1) * num_single_tactile_patches] += self.encoder_modality_embedding(
                        torch.tensor(1 + i, device=device))
            if self.use_sincosmod_encodings:
        ...
        masked_indices_image, unmasked_indices_image = rand_indices_image[:, :num_masked_image], rand_indices_image[:,
                                                                                                 num_masked_image:]
        ...
        if self.num_tactiles > 0 and use_tactile:
            ...
            for i in range(self.num_tactiles):
                rand_indices_tactile = torch.rand(batch, num_tactile_patches // self.num_tactiles,
                                                  device=device).argsort(dim=-1) + count
        ...
        if self.num_tactiles > 0 and use_tactile:
            ...
            for i in range(self.num_tactiles):
                if self.use_sincosmod_encodings:
                    decoder_tactile_tokens[:, i * num_single_tactile_patches:(
                                                                                         i + 1) * num_single_tactile_patches] += self.decoder_modality_embedding(
                        torch.tensor(1 + i, device=device))
            if self.use_sincosmod_encodings:
    ...
    def reconstruct(self, x, mask_ratio=None, use_vision=True, use_tactile=True):
        ...
        if not self.early_conv_masking:  # Hack: see forward() for explanation
            ...
            masked_tactile_patches = tactile_patches[
                batch_range, masked_indices_tactile - image_patches.shape[1]].clone()
        ...
        if use_vision:
            image_transform = Rearrange('b (h w) (p1 p2 c) -> b c (h p1) (w p2)',
                                        h=self.encoder.image_height // self.encoder.image_patch_height,
                                        w=self.encoder.image_width // self.encoder.image_patch_width,
        ...
        if self.num_tactiles > 0 and use_tactile:
            ...
            tactile_transform = Rearrange('b (n h w) (p1 p2 c) -> b (n c) (h p1) (w p2)',
                                          h=self.encoder.tactile_height // self.encoder.tactile_patch_height,
                                          w=self.encoder.tactile_width // self.encoder.tactile_patch_width,
    ...
    def train_iterations(self, iterations, replay_buffer, no_tactile=False):
        ...
        for i in t:
            ...
            if 'image' in new_x:
                ...
                new_x['image'] = new_x['image'].reshape(
                    (new_x['image'].shape[0], new_x['image'].shape[1], new_x['image'].shape[2], -1))
            if 'tactile' in new_x:
                new_x['tactile'] = new_x['tactile'].reshape(
                    (new_x['tactile'].shape[0], -1, new_x['tactile'].shape[3], new_x['tactile'].shape[4]))
...
class VTT(nn.Module):
    ...
    def __init__(self, *, image_size, tactile_size, image_patch_size, tactile_patch_size, dim, depth, heads, mlp_dim,
                 image_channels=3, tactile_channels=3, dim_head=64, dropout=0., emb_dropout=0, num_tactiles=2,
                 frame_stack=1):
        super().__init__()
        ...
        num_patches_tactile = (tactile_height // tactile_patch_height) * (
                    tactile_width // tactile_patch_width) * num_tactiles
...
class MAEExtractor(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.Space, mae_model, dim_embeddings, vision_only_control,
                 frame_stack) -> None:
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        ...
        if 'image' in observations and len(observations['image'].shape) == 5:
            ...
            observations['image'] = observations['image'].reshape(
                (observations['image'].shape[0], observations['image'].shape[1], observations['image'].shape[2], -1))
        if 'tactile' in observations and len(observations['tactile'].shape) == 5:
            observations['tactile'] = observations['tactile'].reshape((observations['tactile'].shape[0], -1,
                                                                       observations['tactile'].shape[3],
                                                                       observations['tactile'].shape[4]))
...
class MAEPolicy(ActorCriticPolicy):
    ...
    def __init__(
        ...
        features_extractor_kwargs = {'mae_model': mae_model, 'dim_embeddings': dim_embeddings,
                                     'vision_only_control': vision_only_control, 'frame_stack': frame_stack}
        ortho_init = False
